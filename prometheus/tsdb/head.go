...
import (
	...
	"github.com/prometheus/prometheus/tsdb/maas"
...
type Head struct {
	...
	// MaaS allocator for remote memory management
	maasAllocator *maas.ChunkAllocator
}
...
type HeadOptions struct {
	...
	MaaSURL             string // URL of MaaS backend (e.g., "http://localhost:3000")
	MaaSFallbackEnabled bool   // If true, fall back to native allocation when MaaS fails
}
...
func NewHead(r prometheus.Registerer, l *slog.Logger, wal, wbl *wlog.WL, opts *HeadOptions, stats *HeadStats) (*Head, error) {
	...
	if opts.MaaSURL != "" {
		...
		h.maasAllocator = maas.NewChunkAllocator(opts.MaaSURL, l, opts.MaaSFallbackEnabled)
		if err := h.maasAllocator.Initialize(); err != nil {
			if !opts.MaaSFallbackEnabled {
				return nil, fmt.Errorf("failed to initialize MaaS allocator: %w", err)
			}
			l.Warn("Failed to initialize MaaS, continuing with native allocation", "error", err)
		} else {
			// Set global allocator for chunk encoding
			chunkenc.SetMaaSAllocator(h.maasAllocator)
			l.Info("MaaS allocator initialized successfully")
		}
	}
...
func (h *Head) Close() error {
	...
	// Cleanup MaaS allocations
	if h.maasAllocator != nil {
		if err := h.maasAllocator.Cleanup(); err != nil {
			h.logger.Warn("Failed to cleanup MaaS allocations", "error", err)
