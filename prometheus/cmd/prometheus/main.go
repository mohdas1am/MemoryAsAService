...
func main() {
	...
	serverOnlyFlag(a, "storage.tsdb.maas.url",
		"URL to Memory-as-a-Service backend for extended memory allocation. Example: http://localhost:3000").
		Default("").StringVar(&cfg.tsdb.MaaSURL)

	serverOnlyFlag(a, "storage.tsdb.maas.fallback",
		"Enable fallback to local memory if MaaS backend is unavailable or allocation fails.").
		Default("true").BoolVar(&cfg.tsdb.MaaSFallbackEnabled)
...
type tsdbOptions struct {
	...
	MaaSURL             string // URL to MaaS backend (e.g., http://localhost:3000)
	MaaSFallbackEnabled bool   // Enable fallback to native memory if MaaS fails
}
...
func (opts tsdbOptions) ToTSDBOptions() tsdb.Options {
	return tsdb.Options{
		...
		MaaSURL:                        opts.MaaSURL,
		MaaSFallbackEnabled:            opts.MaaSFallbackEnabled,
	}
}
